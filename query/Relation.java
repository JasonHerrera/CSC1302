...
public class Relation {
  ...
  public Relation times(Relation r2) {
    ...
    for (int i = 0; i < this.table.size(); i++) {
	    for (int j = 0; j < r2.table.size(); j++) {
	      Tuple newTuple = this.table.get(i).concatenate(r2.table.get(j),
								                                        newAttr, newDoms);
  ...
  // returns a Relation whose Tuples are formed by projecting columns from cnames
  public Relation project(ArrayList<String> cnames) {
    String newName = "PROJECT_";
    for (int i = 0; i < cnames.size(); i++) {
	    newName = newName + cnames.get(i) + "_";
    }
    ...
    for (int i = 0; i < cnames.size(); i++) {
	    for (int j = 0; j < domains.size(); j++) {
	      if (cnames.get(i).equals(this.attributes.get(j))) {
		      newDoms.add(this.domains.get(j));
		      break;
	      }
	    }
    }

    Relation newRelation = new Relation(newName, newAttr, newDoms);
    for (int i = 0; i < this.table.size(); i++) {
	    Tuple newTuple = table.get(i).clone(attributes);
	    newRelation.addTuple(newTuple.project(cnames));
    }
    newRelation.removeDuplicates();
    return newRelation;
  }

  // Week 8

  // takes a comparison condition explained in Tuple
  // returns a new Relation that contains only those tuples
  // that satisfy the comparison conditions
  public Relation select(String lopType, String lopValue, String comparison,
  		                    String ropType, String ropValue) {
    String newName = "SELECT_" + lopValue + "_" + comparison + "_" + ropValue;
    Relation newRel = new Relation(newName, attributes, domains);
    for (int i = 0; i < table.size(); i++) {
      if (table.get(i).select(lopType, lopValue, comparison,
	  			                    ropType, ropValue)) {
	      newRel.addTuple(table.get(i));
	    }
    }
    return newRel;
  }
  ...
  // combines two Relations into one based on common columns in the two relations
  // schema is 1st Relation schema followed by 2nd Relation schema w/o common
  // columns. Two tuples will join only if they have the same values under
  // the common columns.
  public Relation join(Relation r2) {
    ...
    for (int i = 0; i < r2.attributes.size(); i++) {
      if (!newAttr.contains(r2.attributes.get(i))) {
	      newAttr.add(r2.attributes.get(i));
	      newDoms.add(r2.domains.get(i));
	    }
    }

    ArrayList<String> test = new ArrayList<String>();
    test.addAll(this.attributes);
    test.addAll(r2.attributes);
    if (test.equals(newAttr))
      return this.times(r2);

    String newName = this.name + "_JOIN_" + r2.name;
    Relation newRel = new Relation(newName, newAttr, newDoms);
    for (int i = 0; i < this.table.size(); i++) {
      for (int j = 0; j < r2.table.size(); j++) {
	      Tuple t1 = this.table.get(i).clone(this.attributes);
	      Tuple t2 = r2.table.get(j).clone(r2.attributes);
	      Tuple newTup = t1.join(t2, newAttr, newDoms);
	      if (newTup != null)
