...
public class Tuple {
  ...
  public Tuple concatenate(Tuple t2, ArrayList<String> attrs,
  ...
  public boolean select(String lopType, String lopValue, String comparison,
    ...
    if (lopType.equals("num") && ropType.equals("num")) {
      ...
      if (domains.get(lSpot).equals("VARCHAR")) {
        String lVal = (String)tuple.get(lSpot);
        String rVal = (String)tuple.get(rSpot);
  	    int comp = lVal.compareTo(rVal);
  	    switch (comparison) {
  	      case "=":
  	        if (comp == 0)
  		        return true;
      ...
      if (domains.get(lSpot).equals("INTEGER")) {
        Integer lVal = (Integer)tuple.get(lSpot);
        Integer rVal = (Integer)tuple.get(rSpot);
  	    int comp = lVal.compareTo(rVal);
  	    switch (comparison) {
  	      case "=":
  	        if (comp == 0)
      ...
      if (domains.get(lSpot).equals("DECIMAL")) {
        Double lVal = (Double)tuple.get(lSpot);
        Double rVal = (Double)tuple.get(rSpot);
  	    int comp = lVal.compareTo(rVal);
  	    switch (comparison) {
  	      case "=":
  	        if (comp == 0)
  ...
  // attempts to construct a joined Tuple out of this and t2
  // if the 2 Tuples can join, the joined Tuple is returned;
  ...
  public Tuple join(Tuple t2, ArrayList<String> attr, ArrayList<String> dom) {
    ...
    for (int i = 0; i < leftJoinColumns.size(); i++) {
      int leftIndex = leftJoinColumns.get(i);
	    int rightIndex = rightJoinColumns.get(i);
	    if (!this.tuple.get(leftIndex).equals(t2.tuple.get(rightIndex)))
	      return null;
    }

    Tuple newTuple = new Tuple(attr, dom);
    newTuple.tuple.addAll(this.tuple);

    for (int i = 0; i < t2.tuple.size(); i++) {
      if (!rightJoinColumns.contains(i))
	    newTuple.tuple.add(t2.tuple.get(i));
    }
    return newTuple;
  }
}
